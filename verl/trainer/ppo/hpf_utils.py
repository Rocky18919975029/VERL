...
def build_hpf_masked_batches(
    ...
    This is the minimal masked-GRPO version of HPF-RLVR. It assumes the rollout
    batch already contains full responses generated by the current policy. The
    prefix horizon follows h_r=min(r*B_prog,L). Leader reward is any-correct
    within each prefix group; because standard GRPO rollout does not yet contain
    explicit prefix tree ids, the default prefix group is each sampled response.
    The full N-by-M tree rollout is added in a later step.
    """
    ...
    # Minimal Step-4 grouping:
    # - follower: compare rollouts from the same original prompt uid
    # - leader: use any-correct prefix reward; until tree rollout is wired into
    #   trainer, each sampled response is its own prefix candidate under uid.
    follower_adv = _normalize_group_scores(rewards, uid, epsilon, std_normalize)
    prefix_ids = np.arange(len(uid), dtype=object)
    leader_prefix_group = _group_ids(uid, prefix_ids)
    leader_reward = correct
    leader_adv = _normalize_group_scores(leader_reward, uid, epsilon, std_normalize)
    ...
    leader_batch = HPFMaskedBatch(
        ...
        metrics={
            ...
            "hpf/minimal_grouping": 1.0,
            "hpf/leader_prefix_groups": float(len(np.unique(leader_prefix_group))),
        },
    )
